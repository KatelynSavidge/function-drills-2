...
// CODE HERE 
function contains (arr1, name, callback) {
    let inHere = false
...
// CODE HERE
function printer (UNIQARRPARAM) {
  console.log(`New names in array with duplicates removed ${UNIQARRPARAM}`)
}
...
// CODE HERE 
function getUserById (arr1, id, callback) {
  for (let i = 0; i<arr1.length; i++) {
    if (arr1[i].id === id){
      callback(arr1[i])
    }
